{% test mutually_exclusive_ranges(
    model,
    lower_bound_column,
    upper_bound_column,
    partition_by=None,
...
{% macro default__test_mutually_exclusive_ranges(
    model,
    lower_bound_column,
    upper_bound_column,
    partition_by=None,
    ...
    with
        window_functions as (
            ...
            select
                ...
                lead({{ lower_bound_column }}) over (
                    {{ partition_clause }} order by {{ lower_bound_column }}
                ) as next_lower_bound,
        ...
        calc as (
            -- We want to return records where one of our assumptions fails, so we'll
            -- use
            -- the `not` function with `and` statements so we can write our
            -- assumptions nore cleanly
            select
                *,
