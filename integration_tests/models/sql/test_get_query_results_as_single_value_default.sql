...
with default_data as (
    ...
    select
        ...
        cast({{ dbt.string_literal(dbt_utils.get_query_results_as_single_value(false_statement, '2022-01-01')) }} as {{ dbt.type_timestamp() }}) as date_actual,
        ...
        cast({{ dbt.string_literal(dbt_utils.get_query_results_as_single_value(false_statement, 'fallback')) }} as {{ dbt.type_string() }}) as string_actual
