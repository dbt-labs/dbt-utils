...
(
    {{ dbt_utils.datediff(first_date, second_date, "year") }} * 4
    + date_part('quarter', ({{ second_date }})::date)
    - date_part('quarter', ({{ first_date }})::date)
)
{% elif datepart == "month" %}
(
    {{ dbt_utils.datediff(first_date, second_date, "year") }} * 12
...
{{
    exceptions.raise_compiler_error(
        "Unsupported datepart for macro datediff in postgres: {!r}".format(datepart)
    )
}}
{% endif %}
