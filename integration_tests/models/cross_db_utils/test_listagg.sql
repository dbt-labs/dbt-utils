with
    data as (select * from {{ ref("data_listagg") }}),

    data_output as (select * from {{ ref("data_listagg_output") }}),

    calculate as (

        select
            group_col,
            {{ dbt_utils.listagg("string_text", "'_|_'", "order by order_col") }}
            as actual,
        ...
        select
            group_col,
            {{ dbt_utils.listagg("string_text", "'_|_'", "order by order_col", 2) }}
            as actual,
...
select calculate.actual, data_output.expected
from calculate
left join
    data_output
