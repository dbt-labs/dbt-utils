...
{% macro default__get_period_boundaries(
    target_schema,
    target_table,
    timestamp_field,
    start_date,
...
{% materialization insert_by_period, default -%}
    ...
    {%- set start_timestamp = (
        load_result("period_boundaries")["data"][0][0] | string
    ) -%}
    ...
    {% for i in range(num_periods) -%}
        {%- set msg = (
            "Running for " ~ period ~ " " ~ (i + 1) ~ " of " ~ (num_periods)
        ) -%}
        {{ dbt_utils.log_info(msg) }}

        {%- set tmp_identifier = (
            model["name"] ~ "__dbt_incremental_period" ~ i ~ "_tmp"
        ) -%}
      ...
      (
        ...
        {%- if loop_vars.update({"sum_rows_inserted": sum_rows_inserted}) %}
        {% endif -%}
